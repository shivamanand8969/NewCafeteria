...
const cart = async () => {
    let data1=await fetch(`${process.env.NEXT_PUBLIC_NOT_SECRET_MESSAGE}/api/cartitems`,{cache:"no-store"});
    ...
    let handledelete=async(fordata)=>{
      ...
      let id=fordata.get('id');
        ...
        let delet=await fetch(`${process.env.NEXT_PUBLIC_NOT_SECRET_MESSAGE}/api/cartitems/${id}`,{
          method:"DELETE",
          headers:{
